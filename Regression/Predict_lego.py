...
def crossValidation(xArr, yArr, numVal=10):
    ...
    print('交叉验证岭回归结果：%f%+f*年份%+f*部件数量%+f*是否为全新%+f*原价' % (
...
def useStandRegres():
    ...
    print('标准回归结果：%f%+f*年份%+f*部件数量%+f*是否为全新%+f*原价' % (ws[0], ws[1], ws[2], ws[3], ws[4]))
...
def usesklearn():
    ...
    setDataCollect(lgX, lgY)
    reg.fit(lgX, lgY)
    print('Sklearn搭建线性模型结果: %f%+f*年份%+f*部件数量%+f*是否为全新%+f*原价' % (reg.intercept_, reg.coef_[0], reg.coef_[1], reg.coef_[2], reg.coef_[3]))


def useCrossValidation():
    lgX = []
    lgY = []
    setDataCollect(lgX, lgY)

...
def splitDataSet(dataSet, axis, value):
    ...
    for featVec in dataSet:  ##遍历数据集
        if featVec[axis] == value:
            reducedFeatVec = featVec[:axis]  # 删掉axis的特征，保留剩下的特征并存到retDataSet 少了featVec[axis]这一项
...
def chooseBestFeatureToSplit(dataSet):
    ...
    for i in range(numFeatures):  # 遍历所有特征
        ...
        uniqueVals = set(featList)  # 去重复值 即确定该特征有多少种取值
        newEntropy = 0.0
...
def majorityCnt(classList):
    ...
    for vote in classList:
        if vote not in classCount.keys(): classCount[vote] = 0#将第一次出现的类标签加入classCount
        classCount[vote] += 1#出现了一次，对其加一
...
def getNumLeafs(myTree):
    ...
    firstStr = list(myTree.keys())[0]#获取当前节点的第一个特征值
    secondDict = myTree[firstStr]#获取该特征值对应的子树
    for key in secondDict.keys():
        if type(secondDict[key]).__name__ == 'dict':  # 测试该结点是否为字典，如果不是字典，代表此结点为叶子结点
            numLeafs += getNumLeafs(secondDict[key])#递归加入该子树的叶子结点
        else:
            numLeafs += 1#为叶子结点所以加一
    return numLeafs
...
def classify(inputTree, featLabels, testVec):
    ...
    secondDict = inputTree[firstStr]  # 该节点的子树 下一个字典
    featIndex = featLabels.index(firstStr)
    for key in secondDict.keys():
        if testVec[featIndex] == key:
            if type(secondDict[key]).__name__ == 'dict':#如果该节点为非叶节点则进入其子树继续分类
